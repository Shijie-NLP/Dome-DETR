...
- ``HybridEncoder`` is D-FINE's hybrid encoder: a channel projection per backbone level, an
  intra-scale transformer on the listed (top) levels, and a CSP-ELAN feature pyramid across
  levels. It differs from upstream only in upsampling to the finer level's exact size, which a
  four-level pyramid on arbitrary input sizes needs. It has one hook, ``enhance``, that runs on
  the projected levels before the transformer and may add entries to the output dict; here it
  does nothing, so this is the baseline to build on.
- ``DomeHybridEncoder`` fills that hook with the two Dome additions: DeFE, a light density head
  on the stride-4 level that predicts a per-pixel object density map and a per-image count (the
  criterion supervises the map with a Gaussian heatmap drawn from the boxes; the decoder sizes
  its query budget from the pooled map), and MWAS, a window attention over the stride-8 level
  restricted to the windows the density map marks as populated.
"""
...
__all__ = ["HybridEncoder", "DomeHybridEncoder"]
