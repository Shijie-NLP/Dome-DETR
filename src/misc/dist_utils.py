...
def setup_distributed(
    ...
    try:
        ...
        torch.distributed.init_process_group(init_method="env://", timeout=datetime.timedelta(seconds=300))
...
def de_model(model):
    return de_parallel(de_compile(model))
...
def check_compile():
    ...
    if not gpu_ok:
        warnings.warn(
            "GPU is not NVIDIA V100, A100, or H100. Speedup numbers may be lower than expected.", stacklevel=2
        )
    return gpu_ok
